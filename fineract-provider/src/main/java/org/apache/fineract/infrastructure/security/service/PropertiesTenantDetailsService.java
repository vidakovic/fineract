...
public class PropertiesTenantDetailsService implements TenantDetailsService {
    ...
    public FineractPlatformTenant loadTenantById(final String tenantIdentifier) {
        FineractPlatformTenant tenant = fineractProperties.getTenants().get(tenantIdentifier);
    ...
    public FineractPlatformTenant loadTenantById(String tenantIdentifier, boolean isReport) {
        return loadTenantById(tenantIdentifier);
    }

    @Override
    public List<FineractPlatformTenant> findAllTenants() {
        if (fineractProperties.getTenants() != null && !fineractProperties.getTenants().isEmpty()) {
            return fineractProperties.getTenants().values().stream().toList();
        }
