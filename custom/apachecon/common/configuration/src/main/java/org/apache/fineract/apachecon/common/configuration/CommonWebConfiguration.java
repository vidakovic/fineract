 ...
 * under the License.
 */
package org.apache.fineract.apachecon.common.configuration;
...
public class CommonWebConfiguration {
    ...
    @PostConstruct
    public void init() {
        log.warn(">>>>>>>>>>>>>>>>>> Community over Code 2024: Note web configuration...");
    }

    @Bean
    @Order(1)
    public SecurityFilterChain cocFilterChain(HttpSecurity http) throws Exception {
        http.securityMatcher(antMatcher("/coc/**")).authorizeHttpRequests((auth) -> {
            auth.requestMatchers(antMatcher(HttpMethod.OPTIONS, "/coc/**")).permitAll().anyRequest().fullyAuthenticated();
        ...
        if (serverProperties.getSsl().isEnabled()) {
            http.requiresChannel(channel -> channel.requestMatchers(antMatcher("/coc/**")).requiresSecure());
        }
    ...
    public TenantAwareBasicAuthenticationFilter tenantAwareBasicAuthenticationFilter() {
        ...
        filter.setRequestMatcher(antMatcher("/coc/**"));
        return filter;
    }
}
