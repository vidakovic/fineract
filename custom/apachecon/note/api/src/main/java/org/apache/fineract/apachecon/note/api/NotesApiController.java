...
import org.apache.fineract.apachecon.command.service.CommandDispatcher;
import org.apache.fineract.apachecon.note.data.NoteCreateRequest;
...
public class NotesApiController {
    ...
    public NoteResultData addNewNote(@PathVariable("resourceType") @Parameter(description = "resourceType") final String resourceType,
                                     @PathVariable("resourceId") @Parameter(description = "resourceId") final Long resourceId,
                                     @Parameter(hidden = true) final NoteDetailData data) {

        var command = new NoteCreateRequest();

        // TODO: set more metadata (username, tenant ID etc. etc.)
        command.setBody(data);

        var response = commandDispatcher.dispatch(command);

        return (NoteResultData) response.getBody();
    }
}
