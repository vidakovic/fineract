...
import org.apache.fineract.apachecon.note.data.NoteDeleteRequest;
import org.apache.fineract.apachecon.note.data.NoteDeleteResponse;
...
public class NoteDeleteCommandHandler implements CommandHandler<NoteDeleteRequest, NoteDeleteResponse> {
    ...
    public boolean accept(Class<? extends CommandRequest> clazz) {
        return clazz.equals(NoteDeleteRequest.class);
    }

    @Transactional
    @Override
    public NoteDeleteResponse handle(NoteDeleteRequest request) {
        var result = noteWriteService.deleteNote(request.getBody());

        var response = new NoteDeleteResponse();
