...
import org.apache.fineract.apachecon.note.data.NoteCreateResponse;
...
public class NoteCreateCommandHandler implements CommandHandler<NoteCreateRequest, NoteCreateResponse> {
    ...
    public boolean accept(Class<? extends CommandRequest> clazz) {
        return clazz.equals(NoteCreateRequest.class);
    }

    @Transactional
    @Override
    public NoteCreateResponse handle(NoteCreateRequest request) {
        var result = noteWriteService.createNote(request.getBody());

        var response = new NoteCreateResponse();
